...
In the above example, we have created three different
variables:
...
Notice that we have used the 'println!' macro to print the
variables.
...
By default, Rust variables are immutable, which means we
cannot change the value of a variable once it is defined.
...
This is because we are trying to change the value of the
'x' variable from 1 to 2.
----------------------------------------------------------------
error[E0384]: cannot assign twice to immutable variable `x`
  --> _05_Rust_variables_and_mutability.rs:75:5
   |
71 |     let x = 1;
   |         -
   |         |
...
We use the 'mut' keyword before the variable name to create
a mutable variable. For example,
...
Here, you can see we have successfully changed the value of
the 'x' variable. This is because we have defined this 'x'
variable using the 'mut' keyword.


Rules for Naming Variables in Rust
===================================

We can use any names as variable names, however, there are
some rules we should follow:

1. Rust is a case sensitive language. Hence, lowercase
variables and uppercase variables are different:
...
3. Variable names can only contain letters, digits and an
...
Note: Always try to give meaningful names to your variables.
For example, 'name', 'age', 'number' are better names than n, ag, nm.
...
changed. We use the 'const' keyword to create constants in Rust. */
...
In the above example, we have declared a constant PI with value of
 3.14. Now, the value of PI cannot be changed throughout the program.
...
error[E0070]: invalid left-hand side of assignment
   --> _05_Rust_variables_and_mutability.rs:203:8
    |
203 |     PI = 535.23;
    |     -- ^
    |     |
...
Note: As per Rust's naming convention, we use uppercase for the
name of constants.
